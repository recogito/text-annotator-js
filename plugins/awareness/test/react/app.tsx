import { Annotorious, AnnotoriousPlugin, useAnnotations, useAnnotator } from '@annotorious/react';
import { TextAnnotation, TextAnnotator } from '@recogito/react-text-annotator';
import { mountPlugin as AwarenessPlugin } from '../../src';
import { MockAwareness } from './mock-awareness';
import { useEffect, useState } from 'react';

export const App = () => {

  const [remoteSelection, setRemoveSelection] = useState<string>();

  const anno = useAnnotator();

  const annotations = useAnnotations<TextAnnotation>();

  const onToggleMockAwareness = () => setRemoveSelection(current => {
    if (current) {
      return undefined;
    } else {
      const randomIdx = Math.floor(Math.random() * annotations.length);
      return annotations[randomIdx].id;
    }
  });

  useEffect(() => {
    MockAwareness.select(remoteSelection);
  }, [remoteSelection]);

  useEffect(() => {
    // Legacy init
    if (!anno) return;
    (anno as any)?.setPresenceProvider(MockAwareness.provider);
  }, [anno]);

  return (
    <div>
      <button onClick={onToggleMockAwareness}>
        Simulate Presence
      </button>
      ...
      <AnnotoriousPlugin
        ...
        opts={{ provider: MockAwareness.provider }} />
    </div>
  );
